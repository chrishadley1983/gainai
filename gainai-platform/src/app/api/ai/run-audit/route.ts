...
import { runFullAudit } from '@/lib/ai/audit'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (location.oauth_status !== 'CONNECTED') {
      return NextResponse.json(
        { success: false, error: { code: 'OAUTH_REQUIRED', message: 'Google OAuth connection required to run audit' } },
        { status: 400 }
      )
    }

    const auditResult = await runFullAudit(locationId)

    return NextResponse.json(
      { success: true, data: { audit: auditResult } },
      { status: 200 }
    )
  } catch (error) {
    console.error('AI run audit error:', error)
    return NextResponse.json(
